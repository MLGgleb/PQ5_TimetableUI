...
class MainWindow(QWidget):
    ...
    def _connect_to_db(self):
        self.conn = psycopg2.connect(database="timetabledb",
                                     user="postgres",
                                     password="123456",
                                     host="localhost",
                                     port="5432")
    ...
    def _create_teacher_tab(self):
        ...
        self.teach_table.setHorizontalHeaderLabels(["Полное имя", 'Предметы'])
    ...
    def _create_table(self, table_gbox):
        ...
        self.table.setHorizontalHeaderLabels(["Предметы", "Время", "Неделя", "Join", "Delete"])
    ...
    def _update_table(self, table_gbox):
        self.cursor.execute(
            "SELECT * FROM timetable WHERE day='{}' and (week='{}' OR week='обе')".format(table_gbox,
                                                                                          even_odd()))
    ...
    def _change_day_from_table(self, button):
        ...
        try:
            ...
            self.cursor.execute(
                "UPDATE timetable SET subject='{0}', time='{1}', week='{2}' "
            ...
            # обе, Четверг, Социология, 9:30-11:05
            self.conn.commit()
